...
//function for read a file and tokenize the input string
void readFile(list* listR, int* lengthR){
	FILE *fp;
	char file_name[128];
...
//Handler for the string from fgets (erase line feed & carriage return)
void handleInput(char* lineH){
...
void tokenLine(list* listT, int* lengthT, char* lineT){
	...
	while(token != NULL){
		...
		if((i == *lengthT) && (i < MAX_LIST_LEN) && (strlen(listT[i].word) < MAX_WORD_LEN)){
